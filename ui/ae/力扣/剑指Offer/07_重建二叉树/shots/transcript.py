...
subtitles = {
    ...
    1: [
        '输入某二叉树的前序遍历和中序遍历结果，请构建该二叉树并返回其根节点',
        '假设输入的前序遍历和中序遍历结果中，都不包含重复数字',
    ],
    2: [
        '题目举了两个例子帮助理解题意',
        '示例 1',
    ...
    4: [
        ...
        '否则即使本地调试通过，提交到力扣，依然会因为复杂度太高，而无法通过全部测试用例',
    ],
    5: [
    ...
    8: [
        '前序遍历的根节点是[3]',
        '所以中序遍历根节点的位置就是[3]所在的位置',
        '[9]是左子树，节点数是1，[15,20,7]是右子树，节点数是3',
        '知道了左右子树的节点数，就能定位前序遍历结果的左右子树分布',
        '根节点往后移动1个单位是左子树的区域',
        '继续往后移动3个单位是右子树的区域',
        '重复以上分析就可以递归地划分出左右子树以及子树的子树的节点分布',
        '然后把二叉树重建回来',
    ],
    9: [
        # 过渡语start
    ...
    10: [
        ...
        '把根节点创建出来',
        '根据中序遍历根节点的位置计算出左右子树的节点数',
        '进而定位出前序遍历左右子树的分布',
        '接着把前序遍历和中序遍历的左右子树边界，再分别传入递归重建函数',
        '不断重复以上分析和操作，就能递归地重建所有节点',
